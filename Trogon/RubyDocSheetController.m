...
- (IBAction)add:(id)sender {
    if (objectSheet == nil) {
        ...
        NSNib *nib = [[NSNib alloc] initWithNibNamed:@"RubyDocSheet" bundle:myBundle];
        
        BOOL success = [nib instantiateNibWithOwner:self topLevelObjects:nil];
        if (success != YES) {
            NSError *error;
            NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
            [errorDetail setValue:@"Unable to load RubyDocSheet.xib" forKey:NSLocalizedDescriptionKey];
            error = [NSError errorWithDomain:@"TrogonRubyDocSheetDomainError" code:100 userInfo:errorDetail];
...
- (void)objectSheetDidEnd:(NSWindow *)sheet
    ...
    if (returnCode == NSOKButton) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TrogonAddRubyDoc" 
                                                            object:self
                                                          userInfo:nil];        
    }
    else {
