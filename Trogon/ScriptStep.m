...
- (NSAttributedString *)attributedStringForString:(NSString *)aString
{
	...
	return [[NSAttributedString alloc] initWithString:aString attributes:currentQueue.textAttributes];
}
...
- (NSArray *)resolvedScriptArrayForArray:(NSArray *)argumentsArray
{
	...
	for (id __strong value in argumentsArray)
	{
		if ([value isKindOfClass:[ScriptValue class]])
		{
			if (currentQueue)
...
{
	ScriptValue *scriptArgument = [[ScriptValue alloc] init];
