...
#import "GemSheetController.h"

@implementation GemSheetController
@synthesize txtGem;
...
- (IBAction)add:(id)sender {
    if (objectSheet == nil) {
        ...
        NSNib *nib = [[NSNib alloc] initWithNibNamed:@"GemSheet" bundle:myBundle];
        
        BOOL success = [nib instantiateNibWithOwner:self topLevelObjects:nil];
        if (success != YES) {
            NSError *error;
            NSMutableDictionary *errorDetail = [NSMutableDictionary dictionary];
            [errorDetail setValue:@"Unable to load GemSheet.xib" forKey:NSLocalizedDescriptionKey];
            error = [NSError errorWithDomain:@"TrogonGemSheetDomainError" code:100 userInfo:errorDetail];
...
- (void)objectSheetDidEnd:(NSWindow *)sheet
    ...
    if (returnCode == NSOKButton) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TrogonAddGem" 
                                                            object:self
                                                          userInfo:info];        
    }
    else {
